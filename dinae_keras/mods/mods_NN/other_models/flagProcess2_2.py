...
def flagProcess2_2(dict_global_Params,genFilename,x_data,mask_data):
    ...
    x = keras.layers.Conv2DTranspose(1*DimAE,(x_data.shape[1],x_data.shape[2]),strides=(x_data.shape[1],x_data.shape[2]),padding='same',output_padding=None,kernel_regularizer=keras.regularizers.l2(wl2))(decoder_input)
    x = keras.layers.Dropout(dropout)(x)
    for kk in range(0,2):
