...
def flagProcess2_1(dict_global_Params,genFilename,x_data,mask_data):
    ...
    x = keras.layers.Conv2D(6*DimAE,(x_data.shape[1],x_data.shape[2]),activation='relu', padding='valid',kernel_regularizer=keras.regularizers.l2(wl2))(input_data)
    ...
    x = keras.layers.Conv2DTranspose(2*DimAE,(x_data.shape[1],x_data.shape[2]),strides=(x_data.shape[1],x_data.shape[2]),padding='same',output_padding=None,kernel_regularizer=keras.regularizers.l2(wl2))(decoder_input)
    for kk in range(0,2):
