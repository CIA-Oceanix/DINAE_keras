...
def flagProcess4_Optim1(dict_global_Params,genFilename,x_train,x_train_missing,mask_train,meanTr,stdTr,x_test,x_test_missing,mask_test,lday_test,encoder,decoder,model_AE,DimCAE):
    ...
    for iter in range(iterInit,Niter):
        ...
        # save models
        genSuffixModel=save_Models(dict_global_Params,genFilename,NBProjCurrent,encoder,decoder,iter,gradModel,gradMaskModel,NBGradCurrent)

        # generate some plots
        plot_Figs(dirSAVE,genFilename,genSuffixModel,\
                  x_train,x_train_missing,x_train_pred,rec_AE_Tr,\
                  x_test,x_test_missing,x_test_pred,rec_AE_Tt,iter)
